...
public class Helper {
    ...
    public static void runReboot() {
        if (ShellInterface.isSuAvailable()) ShellInterface.runCommand("sync && umount /sd-ext && reboot");
    }
    ...
    public static void copyScriptFinal() {
        if (ShellInterface.isSuAvailable()) {
            ...
            ShellInterface.runCommand("cp " + S2E_DIR + "/files/script01.sh " + SCRIPT_DIST);
            ShellInterface.runCommand("chmod 0777 " + SCRIPT_DIST);
        }
    }
    ...
    public static boolean checkConfigDir() {
        if (!checkFileExists(S2E_CONFIG_DIR)) {
            if (ShellInterface.isSuAvailable()) ShellInterface.runCommand("mkdir " + S2E_CONFIG_DIR);
