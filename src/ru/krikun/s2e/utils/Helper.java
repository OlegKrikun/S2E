...
public class Helper {
	...
	public static void runReboot() {
		ShellInterface.runCommand("busybox sync && busybox umount /sd-ext && reboot");
	}
	...
	public static boolean checkConfigDir() {
		if (!checkFileExists(S2E_CONFIG_DIR)) {
			ShellInterface.runCommand("busybox mkdir " + S2E_CONFIG_DIR);
			return checkFileExists(S2E_CONFIG_DIR);
		} else return true;
	}

	private static void createFile(String filePath) {
		ShellInterface.runCommand("busybox touch " + filePath);
	}

	private static void deleteFile(String filePath) {
		if (checkFileExists(filePath)) {
			ShellInterface.runCommand("busybox rm " + filePath);
		}
	}

	private static void writeToFile(String filePath, String string) {
		if (checkFileExists(filePath)) {
			ShellInterface.runCommand("busybox echo " + string + " > " + filePath);
		}
	}

	public static void createStatusFile(String target) {
		if(!checkFileExists(SCRIPT_STATUS_DIR)) ShellInterface.runCommand("busybox mkdir " + SCRIPT_STATUS_DIR);
		String statusFile = SCRIPT_STATUS_DIR + "/" + target;
		createFile(statusFile);
	}
